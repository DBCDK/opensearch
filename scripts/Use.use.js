...
use = (function() {
    ...
    return function(name, version) {
        ...
        var ignore = {'EXPORTED_SYMBOLS':true, 
        ...
        if(globals.hasOwnProperty('__ModulesInfo')
           && __ModulesInfo.checkDepAlreadyLoaded(name)) { 
            return;
        }
        ...
        //
        // rethrow possible exception to get file name etc. with error
        // as the native error doesn't include that.
        try {
            __builtin_use(name);
        } catch ( e ) {
            throw e;
        ...
        if(!globals.hasOwnProperty('EXPORTED_SYMBOLS') 
                || typeof EXPORTED_SYMBOLS !== 'object' 
                || EXPORTED_SYMBOLS.constructor !== Array) {
        ...
        if(version) {
           var versionerror = "Requesting version " + uneval(version) 
                + " of module " + name 
                + " which is version " + uneval(VERSION);
           function splitversion(v) {
                 var result = (typeof(v) === "string") 
                    && v.split(".").map(
           ...
           if(!(moduleversion[0] == version[0] 
                && (moduleversion[1] > version[1] 
                    || (moduleversion[1] === version[1] 
                        && moduleversion[2] >= version[2])))) {
             throw versionerror;
           }
        }
        ...
        for(i=0; i < EXPORTED_SYMBOLS.length; ++i) {
            ...
            if(globals.hasOwnProperty('print') 
                    && !globals.hasOwnProperty(elem)) {
                print('Warning: module "' + name 
                        + '" says it exports global "' 
                        + elem + '" which does not exist\n');
            }
        ...
        for(elem in globals) {
            if(globals.hasOwnProperty(elem)) {
                ...
                if(!exports.hasOwnProperty(elem)) {
                    ...
                    } else if(globals.hasOwnProperty('print') 
                            && !ignore[elem]) {
                        print('Warning: module "' + name 
                            + '" creates non-exported global variable ' 
                            + elem + '\n');
