...
//		} else if ( String(analysisXML.dkabm::record.ac::source).match(/Litteratursiden/) && String(analysisXML.dkabm::record.dc::title).match(/Analyse af/) ) {
  ...
  that.hasOnlineAccess = function( xml, pid ) {
    ...
    var child;
    if (String(manifestationXML.dkabm::record.ac::source).match(/Litteratursiden|Faktalink|Forfatterweb|Spil og Medier|Netlydbog|eReolen|Safari Books Online|Historisk Atlas/)) {
      for each (child in manifestationXML.dkabm::record.dc::identifier) {
		...
		} else if (String(manifestationXML.dkabm::record.ac::source).match(/Samfundsfaget|Religionsfaget|Historiefaget|Danske Dyr|Verdens Dyr/)) {
