...
def snappuller_enable( solr_root ):
    """
    Enables snappulling by placing a file called snappuller-enabled in
    ${solr_root}/logs
    """
    enable_file = os.path.join(solr_root, "logs", "snappuller-enabled")
    if os.path.exists( enable_file ):
        print "snappuller already enabled"
    else:
        f = open(enable_file, 'w')
        f.close()
        print "snappuller enabled"
